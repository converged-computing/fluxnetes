...
func (podGroupManager *PodGroupManager) AskFlux(
	...
	// but then the job restarted, and new pods are present but fluxnetes had assigned nodes to
	...
	// cancel in fluxnetes. What we can do here is assume the previous pods are no longer running
	...
	grpcclient := pb.NewFluxionServiceClient(conn)
...
func (podGroupManager *PodGroupManager) UpdatePod(oldObj, newObj interface{}) {
	...
	case corev1.PodSucceeded:
		podGroupManager.log.Info("[PodGroup UpdatePod] Pod %s succeeded, Fluxnetes needs to free the resources", newPod.Name)
		...
		// be a mistake - fluxnetes would schedule it again
	...
	case corev1.PodFailed:
		...
		podGroupManager.log.Warning("[PodGroup UpdatePod] Pod %s in group %s failed, Fluxnetes needs to free the resources", newPod.Name, groupName)
...
func (podGroupManager *PodGroupManager) DeletePod(podObj interface{}) {
	...
	case corev1.PodSucceeded:
	case corev1.PodPending:
		podGroupManager.log.Verbose("[PodGroup DeletePod] Pod %s completed and is Pending termination, Fluxnetes needs to free the resources", pod.Name)
